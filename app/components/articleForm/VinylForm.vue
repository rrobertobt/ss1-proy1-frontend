    ...
    <form @submit="onSubmit" class="pt-2 pb-14">
      ...
      <fieldset class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" :disabled="loading">
        ...
        <FormField
          ...
          name="type_details.vinyl_category_id"
        >
          <FormItem>
            <FormLabel icon="carbon:category">Categoría de vinilo</FormLabel>
            <FormControl>
              <Select v-bind="componentField" placeholder="Categoría de vinilo">
                <SelectTrigger class="w-full">
                  <SelectValue placeholder="Categoría de vinilo" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem
                    v-for="category in vinylCategories?.data"
                    :key="category.id"
                    :value="category.id"
                  >
        ...
        <FormField
          v-slot="{ componentField }"
          name="type_details.vinyl_special_edition_id"
        >
          <FormItem>
            <FormLabel icon="carbon:star">Edición especial de vinilo</FormLabel>
            <FormControl>
              <Select
                v-bind="componentField"
                placeholder="Edición especial de vinilo"
              >
                <SelectTrigger class="w-full">
                  <SelectValue placeholder="Edición especial de vinilo" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem
                    v-for="edition in vinylSpecialEditions?.data"
                    :key="edition.id"
                    :value="edition.id"
                  >
                    {{ edition.name }}
        ...
        <FormField v-slot="{ componentField }" name="type_details.rpm">
          <FormItem>
            <FormLabel icon="carbon:time">RPM</FormLabel>
            <FormControl>
              <Input type="number" placeholder="RPM" v-bind="componentField" />
            </FormControl>
            <FormMessage />
          </FormItem>
        </FormField>
        ...
        <FormField
          ...
          name="type_details.is_limited_edition"
        >
          <FormItem class="flex items-center space-x-3 space-y-0">
            <FormControl>
              <Checkbox
            ...
            <FormLabel icon="carbon:star">Edición limitada</FormLabel>
          </FormItem>
        </FormField>

        <FormField
          v-slot="{ componentField }"
          name="type_details.remaining_limited_stock"
        >
          <FormItem>
            <FormLabel icon="carbon:box"
              >Stock restante de edición limitada</FormLabel
            >
            <FormControl>
              <Input
                type="number"
                placeholder="Stock restante de edición limitada"
                v-bind="componentField"
              />
            </FormControl>
            <FormMessage />
...
<script setup lang="ts">
  ...
  const { data: vinylCategories } = await useAsyncData(
    () => $api("/catalog/vinyl-categories"),
    {
      lazy: true,
    },
  );

  const { data: vinylSpecialEditions } = await useAsyncData(
    () => $api("/catalog/vinyl-special-editions"),
    {
      lazy: true,
    },
  );
  ...
  const formSchema = yup.object({
    ...
    type_details: yup
      .object({
        vinyl_category_id: yup
          .number()
          .typeError("El ID de la categoría de vinilo debe ser un número")
          .required("El ID de la categoría de vinilo es obligatorio")
          .integer("El ID de la categoría de vinilo debe ser un número entero")
          .positive(
            "El ID de la categoría de vinilo debe ser un número positivo",
          ),
        vinyl_special_edition_id: yup
          .number()
          .typeError(
            "El ID de la edición especial de vinilo debe ser un número",
          )
          .optional()
          .integer(
            "El ID de la edición especial de vinilo debe ser un número entero",
          )
          .positive(
            "El ID de la edición especial de vinilo debe ser un número positivo",
          )
          .nullable(),
        rpm: yup
          .number()
          .typeError("Las RPM deben ser un número")
          .optional()
          .positive("Las RPM deben ser un número positivo"),
        is_limited_edition: yup
          .boolean()
          .typeError("El campo de edición limitada debe ser verdadero o falso")
          .optional(),
        remaining_limited_stock: yup
          .number()
          .typeError("El stock restante de edición limitada debe ser un número")
          .optional(),
      })
