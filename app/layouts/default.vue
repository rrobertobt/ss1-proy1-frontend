<template>
  <div>
    <header
      class="fixed top-0 inset-x-0 z-50 border-b border-foreground/10 backdrop-blur-sm transition-colors"
      ...
      <nav aria-label="principal" class="mx-auto max-w-6xl px-4">
        <div class="h-14 flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            >
              <Icon
                :name="mobileOpen ? 'carbon:close' : 'carbon:menu'"
                size="22"
              />
        ...
        <transition name="fade">
          <ul
            v-if="mobileOpen"
            id="mobile-menu"
            class="md:hidden py-2 border-t border-foreground/10"
          >
            <li v-for="item in items" :key="item.to">
              <NuxtLink
                :to="item.to"
